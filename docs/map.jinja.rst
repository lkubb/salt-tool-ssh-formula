...
We will see a quick setup to configure the ``tool_ssh`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_ssh`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_ssh`` formulas under ``/srv/salt/tool_ssh/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_ssh/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_ssh/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_ssh/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_ssh/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_ssh`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_ssh`` formulas under ``/srv/salt/tool_ssh/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_ssh/parameters/roles
...
We create a configuration for the role ``tool_ssh/server`` in ``/srv/salt/tool_ssh/parameters/roles/tool_ssh/server.yaml``:
...
We create another configuration for the role ``tool_ssh/client`` in ``/srv/salt/tool_ssh/parameters/roles/tool_ssh/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_ssh/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_ssh/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_ssh/parameters/dns:domain/  '/srv/salt/tool_ssh/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-ssh-as-json:
      test.nop:
        - name: {{ ssh | json }}
...
.. code-block:: sls
    ...
    ssh-config-file-file-managed:
      file.managed:
        - name: {{ ssh.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='ssh-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``ssh`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ ssh['winner'] }}
